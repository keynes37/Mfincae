...
`r fa("laptop", fill="red")` Sea un proceso AR(1) de una serie de tiempo de cierta variable $Y_t=2+0.3Y_{t-1}+\epsilon_t$, donde $\epsilon_{t-1}$ que es el residuo o error sigue un proceso ruido blanco $\epsilon_t \sim (0,\sigma^{2}_e)$. La media $E(Y_t)$ de la serie de tiempo puede ser establecida como:
